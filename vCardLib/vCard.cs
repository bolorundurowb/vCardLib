...
namespace vCardLib
{
	...
	public class vCard : IDisposable
	{
		...
		{
			get { return _formattedName ?? ProposeFN(); }
		...
		private string ProposeFN()
		{
			var nameParts = new List<string>();

			if ( !String.IsNullOrEmpty( Prefix?.Trim() ) )		nameParts.Add( Prefix );
			if ( !String.IsNullOrEmpty( GivenName?.Trim() ) )	nameParts.Add( GivenName );
			if ( !String.IsNullOrEmpty( MiddleName?.Trim() ) )	nameParts.Add( MiddleName );
			if ( !String.IsNullOrEmpty( FamilyName?.Trim() ) )	nameParts.Add( FamilyName );

			var proposedName = String.Join( " ", nameParts );

			if ( !String.IsNullOrEmpty( Suffix?.Trim() ) )
				proposedName += $", {Suffix?.Trim()}";
