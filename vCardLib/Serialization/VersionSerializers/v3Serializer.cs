...
internal sealed class V3Serializer
{
    ...
    public string Serialize(vCard card)
    {
        ...
        VCardSerializationFormatting.AppendContentLine(builder, VersionFieldSerializer.Write(vCardVersion.v3));
        ...
        if (card.Members.Any())
            VCardSerializationFormatting.AppendContentLine(builder,
                ((IV3FieldSerializer<List<string>>)_fieldSerializers["MEMBER"]).Write(card.Members)
            );
