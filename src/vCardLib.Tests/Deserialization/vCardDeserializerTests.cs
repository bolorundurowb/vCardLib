...
public class vCardDeserializerTests
{
    ...
    [Test]
    public void FromContent_LeadingBlankLinesBeforeBegin_Parses()
    {
        var content = "\n\nBEGIN:VCARD\nVERSION:4.0\nFN:Leading\nEND:VCARD";
        var vcards = vCardDeserializer.FromContent(content).ToList();
    ...
    [Test]
    public void FromContent_TrailingBlankLinesAfterEnd_Parses()
    {
        var content = "BEGIN:VCARD\nVERSION:4.0\nFN:Trailing\nEND:VCARD\n\n";
        var vcards = vCardDeserializer.FromContent(content).ToList();
    ...
    [Test]
    public void FromContent_TwoCards_CrOnlyDelimiters_ReturnsBoth()
    {
        var content = "BEGIN:VCARD\rVERSION:3.0\rFN:One\rEND:VCARD\rBEGIN:VCARD\rVERSION:3.0\rFN:Two\rEND:VCARD";
        var vcards = vCardDeserializer.FromContent(content).ToList();
        vcards.Count.ShouldBe(2);
        vcards[0].FormattedName.ShouldBe("One");
        vcards[1].FormattedName.ShouldBe("Two");
    ...
    [Test]
    public void FromContent_StrictCrlfSerializedVCard_RoundTripsFormattedName()
    {
